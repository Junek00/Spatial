...
import { X, Check, Pin, RefreshCw, ChevronDown, ChevronRight, ChevronLeft, Link as LinkIcon, Sparkles, Tag, FolderInput, ArrowRightFromLine, Copy, ArrowRight } from "lucide-react"
...
export const TileCard = memo(function TileCard({ 
  ...
  return (
    <div
      ref={cardRef}
      className={`group relative flex h-full w-full flex-col overflow-hidden border transition-all duration-200 min-h-[9rem] ${
    ...
    >
      ...
      >
        <div className={`flex items-center gap-2 overflow-hidden ${isTextRTL ? 'flex-row-reverse' : ''}`} style={{ color: "inherit" }}>
          ...
          <span className="font-mono text-xs font-bold uppercase tracking-wider truncate max-w-[5rem]" title={config.label}>
            {config.shortBadge}
          </span>

          {block.isUnrelated && !effectiveCollapsed && (
            <span className="ml-1 rounded-sm bg-muted px-1.5 py-0.5 font-mono text-xs font-black uppercase tracking-tighter text-muted-foreground">
              주제 무관
            </span>
          )}
        </div>
        <div className={`flex items-center gap-2 flex-shrink-0 ${isTextRTL ? 'flex-row-reverse' : ''}`} style={{ color: "inherit" }}>
          ...
          <span className="font-mono text-xs font-medium opacity-70">
            {formattedTime}
          </span>
      ...
      {isTypePickerOpen && pickerRect && onChangeType && isMounted && createPortal(
        ...
        >
          <p className="px-2.5 pt-2 pb-1 font-mono text-xs uppercase tracking-widest text-muted-foreground/50">
            타입 변경
          </p>
      ...
      {effectiveCollapsed && (
        <div className="flex-1 px-3 py-1.5 overflow-hidden">
          <p className="text-sm leading-relaxed font-medium text-foreground/50 line-clamp-2 italic">
            {block.text}
          </p>
        </div>
      )}
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
              ...
              <div className="flex flex-col min-h-full">
                {isEditing ? (
                  ...
                  <div className="w-full">
                    {block.isError && (
                      <div className="mb-3 flex items-start gap-2 border-l-2 border-destructive/40 bg-destructive/5 px-2.5 py-2">
                        <span className="mt-px font-mono text-[9px] text-destructive/70 uppercase tracking-wider leading-relaxed">
                          {block.statusText === "no-api-key"
                            ? <>API 키 없음 — <strong>☰ 설정</strong>에서 추가하세요.</>
                            : block.statusText === "rate-limit"
                            ? "요청 한도 초과(429). 잠시 후 재시도하세요."
                            : block.statusText === "network-error"
                            ? "네트워크 오류. 연결 상태를 확인하세요."
                            : block.statusText === "server-error"
                            ? "AI 서버 오류(5xx). 잠시 후 재시도하세요."
                            : block.statusText === "json-error"
                            ? "응답 형식 오류 — 이 모델이 JSON 출력을 지원하지 않을 수 있습니다."
                            : block.statusText?.startsWith("error:")
                            ? block.statusText.slice(7)
                            : "분석 실패. 더블클릭으로 재시도하세요."}
                        </span>
                      </div>
                    )}
                      ...
                      {isTask && block.subTasks ? (
                        <div className="flex flex-col gap-1.5">
                          {block.subTasks.map(st => (
                            <div key={st.id} className="group/task flex items-start gap-3 rounded-md bg-muted/30 py-1.5 px-2 transition-colors hover:bg-muted/50">
                              <button
                              ...
                              >
                                {st.isDone && <Check className="h-3 w-3 text-primary-foreground" />}
                              </button>
                    ...
                    {isEditingAnnotation ? (
                        <div className="flex flex-1 w-full flex-col gap-2">
                          ...
                          <div className="flex items-center justify-between px-1">
                            ...
                            <span className="font-mono text-xs text-primary font-bold uppercase tracking-widest opacity-80">마크다운 편집기</span>
                          </div>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-2">
                          <div className={`prose-sm dark:prose-invert max-w-none text-sm leading-relaxed text-foreground/80 ${block.isEnriching ? "shimmer-body" : ""} ${isAnnotationRTL ? 'rtl-text' : ''}`}>
                            <ReactMarkdown 
            ...
            {block.confidence !== undefined && block.confidence !== null && !isEditing && (
              <div className={`px-3 pb-2 flex-shrink-0 transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"}`}>
                <div className="flex items-center justify-between mb-1">
                  <span className="font-mono text-xs text-muted-foreground">신뢰도</span>
                  <span className="font-mono text-xs text-muted-foreground">{Math.round(block.confidence)}%</span>
                </div>
            ...
            <div
              ...
              className={`relative flex flex-shrink-0 flex-col transition-all duration-300 ease-in-out ${
                isFooterExpanded ? "bg-secondary/40" : "bg-muted/20"
              }`}
              style={{
              ...
              <div className={`flex items-start justify-between px-3 py-2 ${isFooterExpanded ? 'gap-3' : 'items-center'}`}>
                <div className={`flex flex-1 items-start gap-2 overflow-hidden ${isFooterExpanded ? 'flex-wrap' : ''}`}>
                  <div className={`flex items-start gap-2 overflow-hidden ${isFooterExpanded ? 'flex-wrap mb-1' : ''}`}>
                    ...
                    <span
                      className="rounded-sm px-2 py-0.5 font-mono text-xs font-bold flex items-center gap-1.5 shadow-sm shrink-0"
                      style={{
                    ...
                    {block.influencedBy && block.influencedBy.length > 0 && (
                      <div className="group/influences relative">
                        ...
                        >
                          ...
                          <span className="font-mono text-xs font-bold text-primary uppercase tracking-tighter">
                            {block.influencedBy.length} 연결
                          </span>
                        </div>

                        {/* Hover Tooltip */}
                        <div className="absolute bottom-full left-0 mb-2 w-56 p-2 rounded-md bg-popover backdrop-blur-md border border-border shadow-xl opacity-0 translate-y-2 pointer-events-none group-hover/influences:opacity-100 group-hover/influences:translate-y-0 transition-all z-[100]">
                          <h5 className="font-mono text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1.5 border-b border-border pb-1">연결된 노드</h5>
                          <div className="flex flex-col gap-1">
                            {block.influencedBy.slice(0, 5).map((id, i) => {
                              ...
                              return (
                                <div key={i} className="flex items-start gap-2 overflow-hidden">
                                  ...
                                  <span className="font-mono text-xs text-foreground/70 truncate leading-tight">
                            ...
                            {block.influencedBy.length > 5 && (
                              <span className="font-mono text-xs text-muted-foreground/50 mt-1">+{block.influencedBy.length - 5}개 더</span>
                            )}
                          </div>
                        </div>
                      </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-sm bg-secondary/30 border border-border/20 transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-20"}`}>
                    <span className="font-mono text-xs text-muted-foreground/50 uppercase tracking-[0.1em]">노드 ID:</span>
                    <span className="font-mono text-xs text-muted-foreground/80 font-bold">#{block.id.slice(0, 6)}</span>
                  </div>
                </div>
              </div>
            </div>
