...
export function TilingArea({
  ...
  const scrollToPage = useCallback((idx: number) => {
    const container = scrollContainerRef.current
    if (!container) return
    const el = container.querySelector<HTMLElement>(`[data-page-idx="${idx}"]`)
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-background">
      {/* Task Header — sticky at top, compact width */}
      {taskBlock && (
        <div className={`shrink-0 p-1 z-10 transition-[opacity,filter] duration-300 ${activeConnectionId && !relatedIds.has(taskBlock.id) ? 'opacity-15 saturate-0' : 'opacity-100'}`}>
        ...
        {pageTrees.length > 0 && (
          <div className="flex flex-col w-full">
            {pageTrees.map((tree, idx) => {
              ...
              const pageHeight = containerHeight > 0
                ? (count <= 2 ? Math.min(300, containerHeight) : containerHeight)
                : undefined
              return (
                <div
                  key={idx}
                  data-page-idx={idx}
                  className="flex w-full border-b border-white/5 last:border-0"
