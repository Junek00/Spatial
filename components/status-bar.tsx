...
export function StatusBar({
  ...
  const typeCounts = useMemo(() => {
    ...
    blocks.forEach((b) => {
      if (b.contentType === "task" && b.subTasks && b.subTasks.length > 0) {
        counts["task"] = (counts["task"] || 0) + b.subTasks.length
      } else {
            ...
            {activity.enriching > 0 && (
              <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-sm bg-primary/10 border border-primary/20">
                ...
                <span className="font-mono text-[10px] text-primary">
                  {activity.enriching} {blocks.length > activity.enriching ? (lang === "ko" ? "문맥 분석 중..." : "Analyzing context...") : (lang === "ko" ? "처리 중..." : "Processing...")}
                </span>
              </div>
            )}
            ...
            {activity.errors > 0 && (
              <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-sm bg-destructive/10">
                ...
                <span className="font-mono text-[10px] text-destructive font-bold">
                  {activity.errors} {lang === "ko" ? "실패" : "failed"}
                </span>
              </div>
            )}
