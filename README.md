...
### 한국어
**요구사항**: 데스크톱 브라우저와 [OpenRouter](https://openrouter.ai) API 키가 필요합니다.
...
**API 키 추가하기**: 좌측 상단 메뉴 아이콘 클릭 → 설정(Settings) → OpenRouter 키를 붙여넣으세요. 키는 브라우저의 `localStorage`에만 저장되며 서버를 거치지 않고 OpenRouter로 직접 전송됩니다.

**웹 검색 기능 활성화** (선택): 설정에서 "Web grounding"을 켜면 AI가 주장, 질문 등에 대해 실제 출처를 인용할 수 있습니다. `:online` 접미사를 지원하는 모델에서 작동합니다.
...
| `inceptionlabs/mercury` | Extremely fast (diffusion-based), large free tier. |
...
| 모델 | 특징 |
|---|---|
| `openai/gpt-4o` | 기본 모델. 주석 품질이 우수함 |
| `anthropic/claude-sonnet-4-5` | 추론 능력이 뛰어나며 복잡한 연구에 적합함 |
| `inceptionlabs/mercury` | 디퓨전 기반의 압도적인 속도, 넉넉한 무료 토큰 제공 |
| `google/gemini-2.5-pro` | 웹 검색 기능 지원 |
| `deepseek/deepseek-chat` | 빠르고 가성비가 좋음 |
...
- 노트는 `localStorage`에 영구 저장됩니다.
- 변경 시마다 조용히 백업됩니다.
- `⌘K`를 눌러 Markdown 또는 JSON으로 내보내기 할 수 있습니다.
- 사이드바에서 `.nodepad` 파일 가져오기가 가능합니다.
