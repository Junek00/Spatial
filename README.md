...
- **Enhanced UI/UX:** 
  - Clickable `Cmd+Z` and `Cmd+K` buttons in the input bar.
  - Interactive `#` autocomplete dropdown to quickly force content types (Entity, Claim, Task, etc.).
  - Detailed error cards specifying failure reasons (Network, Rate Limit, API Key, Server).
  - Tiling view improvements including sticky Task cards, automatic smooth scrolling for new nodes, and accurate page-navigator synchronization.
- **Inception Mercury API Support:** Added support for InceptionLabs Mercury API endpoints.
