...
export function detectScript(text: string): string {
  ...
  if (/[\u4E00-\u9FFF\u3040-\u30FF]/.test(text))               return "Chinese or Japanese"
  ...
  schema: {
    ...
    properties: {
      ...
      confidence: {
        ...
        description: "Integer from 0 to 100 representing confidence percentage. Use null if not applicable.",
      },
      influencedByIndices: {
        type: "array",
        items: { type: "number" },
...
export async function enrichBlockClient(
  ...
  // Gemini and Claude don't support strict json_schema — use json_object / prompt-based JSON
...
You have live web access. For this note type, include 1–2 real source citations by name, publication, and year. Do NOT generate URLs — reference by title and author only (e.g. "Per *Science*, 2023, Doe et al."). Only cite sources you have actually retrieved.`
  ...
  const result_raw = await callAIChat(
    config,
    [
    ...
    ],
    {
      temperature: 0.1,
      jsonMode: true,
      jsonSchema: useStrictSchema ? JSON_SCHEMA : undefined,
    },
    model,
    webSearchOptions,
  )
  ...
  try {
    result = JSON.parse(result_raw.content)
  } catch {
    const fenceMatch = result_raw.content.match(/```(?:json)?\s*\n?([\s\S]*?)\n?\s*```/)
    if (fenceMatch) {
      result = JSON.parse(fenceMatch[1].trim())
    } else {
      throw new Error(
        `AI returned invalid JSON. The model may not support structured output.\n\nRaw response: ${result_raw.content.substring(0, 200)}`
      )
    }
  }
  if (result.confidence != null) {
    // Some models return 0–1 decimal range instead of 0–100 integer range
